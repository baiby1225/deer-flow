...
async def load_config_from_file_async(config_path: str = "mcp.yaml") -> dict:
    ...
    try:
        ...
        # Use asyncio.to_thread to run the file reading operation in a separate thread
        def _read_file():
            with open(config_file, 'r', encoding='utf-8') as f:
                return yaml.safe_load(f)

        config = await asyncio.to_thread(_read_file)
        logger.info(f"MCP configuration loaded from {config_path}")
        return config or {}
    except Exception as e:
        logger.error(f"Error loading configuration from {config_path}: {e}")
