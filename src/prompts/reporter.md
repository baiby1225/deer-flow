...
4. **用户问题答案**
   - **直接回答用户的具体问题**，基于研究结果提供明确的答案。
   - 使用清晰的"问题-答案"格式，确保用户能够快速找到他们关心的问题的答案。
   - 如果用户问题涉及多个方面，分别回答每个方面。
   - 提供具体的、可操作的信息，避免模糊或笼统的回答。
   - 基于研究数据提供准确的答案，如果某些信息无法确定，明确说明。
...
1. 写作风格：
   - **用户问题答案部分**：
     - 必须直接、明确地回答用户提出的具体问题
     - 使用"问题：... 答案：..."的格式，确保清晰对应,**答案部分适当使用表格、列表等更易读的形式**
     - 基于研究数据提供准确、具体的答案
     - 如果问题有多个子问题，分别回答每个子问题
     - 避免使用"可能"、"似乎"等模糊词汇，除非确实无法确定
     - 提供可操作的具体信息，而不是抽象的概念
   ...
   **学术卓越标准：**
   ...
   - 适当使用表格、列表等更易读的形式
   - 不丢失任何细节内容、不过度总结
   - **用户问题答案**：以学术严谨性回答用户问题，提供基于证据的明确结论
   {% elif report_style == "popular_science" %}
   **科学传播卓越标准：**
   - 以对发现的感染性热情和真正好奇心写作
   - 将技术术语转化为生动、相关的类比和隐喻
   ...
   - **用户问题答案**：用通俗易懂的语言直接回答用户问题，让复杂概念变得清晰易懂
   {% elif report_style == "news" %}
   **NBC新闻编辑标准：**
